...
output_folder = fullfile(pwd, 'coloured_traces'); % Save in a folder in the same directory
...
for i = 1:length(land_on.sessions)
    ...
    for d = 1:length(datasets)
        ...
        if use_heatmap
            ...
            % LINE PLOT MODE (All neurons share the same Y-axis with different colors)
            hold on;
            ...
            cmap = lines(N); % MATLAB's built-in "lines" colormap for distinct colors

            for n = 1:N
                plot(1:T, neural_data(:, n), 'Color', cmap(n, :), 'LineWidth', 1);
            end
